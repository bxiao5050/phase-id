...
export default class Type5 extends React.Component<paymentProps, {}, any> {
  ...
  pay = () => {
    RG.jssdk.order(this.props.Payment.state.paymentDatas[this.index]).then(OrderRes => {
  ...
  render() {
    return (
      <div className='payment-nav'>
          ...
          >
            {this.props.Payment.state.paymentDatas[this.index].products.map((key, index) => (
              <option key={index} value={index}>
                {key.gameCoin + key.gameCurrency}
              </option>
            ))}
          </select>
          <a href='javascript:void(0);' className='icon-down'></a>
        </div>
        ...
        <a href='javascript:void(0);' className='buy-goods' onClick={this.pay}>
          Mua KNB
        </a>
      </div>
    );

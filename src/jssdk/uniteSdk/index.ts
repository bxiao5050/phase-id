...
export default class UniteSdk {
  ...
  private quickUserInfo: { uid: string; username: string; token: string; isLogin: boolean; channelId: string; };
  config: JSSDK.Config;
  reactPromise: any;
  reactDomPromise: any;
  quickSdkPromise: any;
  deviceMsg: { source: number, network: number, model: string, operatorOs: string, deviceNo: string, device: string, version: string, };
  constructor(config: any) {
    ...
    this.quickSdkPromise = this.loadScript("https://sdkapi02.quicksdk.net/static/lib/libQuickSDK_v2.js");
  }
  init() {
    this.quickInit().then(() => {

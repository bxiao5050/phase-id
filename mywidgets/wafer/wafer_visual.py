...
class WaferPanel(Frame):
    def __init__(self, master, **kw):
        ...
        #add colored button
        self.pngImage = [ImageTk.PhotoImage(Image.open('yes.png'), master=self), ImageTk.PhotoImage(Image.open('triangle.png'), master=self), ImageTk.PhotoImage(Image.open('rectangle.png'), master=self), ImageTk.PhotoImage(Image.open('no.png'), master=self), ImageTk.PhotoImage(Image.open('default.png'), master=self)]
        for k in range(5):
...
def main():
    root = Tk()
    WaferPanel(root).pack(fill = 'both', expand = True)
